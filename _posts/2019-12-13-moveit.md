...
```c++
fake_ar_publisher::ARMarkerConstPtr p = last_msg_; //订阅来自rosrun fake_ar_publisher fake_ar_publisher_node的话题，这里相当于是获取完毕后的值
if (!p) return false;
...
camera_frame==>base_frame(eg. world 或用户自己指定) 获取方法：按案例假设了已经知道相机的模型，并添加到`workcell.xacro`中
```
