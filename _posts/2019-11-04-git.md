...
|公司gitlab||`git config --global user.name "wumin Wu Min"`<br>`git config --global user.email "wumin@keba.com"`|gitlab只能在公司内网，且绑定电脑Mac地址的。目前我们gitlab也不能用ssh，虽然是管理员权限，目前测试下来，只能用https|
