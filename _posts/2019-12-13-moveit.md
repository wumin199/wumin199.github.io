...
camera_frame==>base_frame(eg. world 或用户自己指定) 获取方法：该案例假设了已经知道相机的模型，并添加到 workcell.xacro 中，是通过tf的lookupTransform获取
...
AR marker轮廓点(contour)在base_frame下的表示  =  (AR marker在base_frame下的表示)     *    (contour在ARMarker坐标系下的表示==>看“模型”那个tab)   
